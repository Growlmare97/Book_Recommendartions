...
def add_flash(db: sqlite3.Connection, category: str, message: str) -> None:
    db.execute(
...
def pop_flash_messages(db: sqlite3.Connection) -> list[sqlite3.Row]:
...
def import_books(db: sqlite3.Connection, books: Iterable[Book]) -> int:
...
def get_or_create_weekly_recommendation(db: sqlite3.Connection, recommendations: list[sqlite3.Row]) -> sqlite3.Row | None:
...
def render_page(params: dict[str, str] | None = None) -> str:
    ...
    with get_db() as db:
        ...
        weekly = get_or_create_weekly_recommendation(db, recommendations)
        flashes = pop_flash_messages(db)

    def esc(text: str) -> str:
        return html.escape(text)

    flash_html = "".join(
        f'<div class="flash {esc(msg["category"])}">{esc(msg["message"])}</div>'
        for msg in flashes
    )
    ...
    books_html = "".join(
        (
            ...
            + esc(book["category"])
            + "</small></div>"
            + f'<form method="post" action="/favourite"><input type="hidden" name="book_id" value="{book["id"]}" /><button type="submit">☆ Favourite</button></form></li>'
        )
        for book in books
    ) or "<li>No books imported yet.</li>"

    favourites_html = "".join(
        (
            "<li><div><strong>"
            + esc(fav["title"])
            + "</strong><br /><small>"
            ...
            + esc(fav["category"])
            + "</small></div>"
            + f'<form method="post" action="/favourite/delete"><input type="hidden" name="fav_id" value="{fav["id"]}" /><button type="submit" class="delete">Remove</button></form></li>'
        )
        for fav in favourites
    ) or "<li>No favourites yet.</li>"

    recommendations_html = "".join(
        f"<li>{esc(rec['title'])} — {esc(rec['author'])} ({esc(rec['category'])})</li>"
...
<div class=\"card\"><h2>Your favourites ({len(favourites)})</h2><ul class=\"book-list\">{favourites_html}</ul></div></section>
<section class=\"card\"><h2>Weekly recommendation</h2>{weekly_html}</section>
<section class=\"card\"><h2>Top recommendations</h2><ol>{recommendations_html}</ol></section></main></body></html>"""
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/static/styles.css":
            ...
            if not css_path.exists():
                ...
                return
            self._send(css_path.read_text(encoding="utf-8"), content_type="text/css; charset=utf-8")
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path == "/import":
            ...
            with get_db() as db:
                try:
                    ...
                    if not books:
                        add_flash(db, "error", "No books found. Make sure your Goodreads profile is public.")
                    else:
                        count = import_books(db, books)
                        add_flash(db, "success", f"Imported {count} books from Goodreads.")
                except Exception as exc:  # noqa: BLE001
                    add_flash(db, "error", f"Unable to import profile: {exc}")
            self._redirect_home()
            return
        ...
        with get_db() as db:
            if self.path == "/favourite":
                ...
                if not book:
                    add_flash(db, "error", "Book not found.")
                else:
                    exists = db.execute(
                    ...
                    if exists:
                        add_flash(db, "info", "Already in favourites.")
                    else:
                        db.execute(
                            """
                        ...
                        add_flash(db, "success", "Added to favourites.")
                self._redirect_home()
                return
            ...
            if self.path == "/favourite/delete":
                ...
                add_flash(db, "info", "Removed favourite.")
                self._redirect_home()
                return
